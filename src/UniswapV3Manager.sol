...
contract UniswapV3Manager is IUniswapV3Manager {
    ...
    function swap(SwapParams memory params) public returns (uint256 amountOut) {
        ...
        while (true) {
            ...
            params.amountIn = _swap(
                params.amountIn,
                // if there are multiple pools in the path, recipient is the manager contract, it’ll store tokens between swaps.
                // If there’s only one pool (last one) in the path, recipient is the one specified in the parameters (usually the same user that initiates the swap).
    ...
    function uniswapV3SwapCallback(
        ...
        // If payer is the current contract (this is so when making consecutive swaps),
        // it transfers tokens to the next pool (the one that called this callback) from current contract’s balance.
